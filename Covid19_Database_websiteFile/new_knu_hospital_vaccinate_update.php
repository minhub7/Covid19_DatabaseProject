...
<?php
        ...
        $sql="SELECT * FROM vaccines, vaccinate WHERE Owned = '경대병원' AND (vaccines.SerialNo = vaccinate.1st_Vno OR vaccines.SerialNo = vaccinate.2nd_Vno) AND Cssn='".$_POST['Cssn']."'";

        $ret=mysqli_query($con,$sql);
        if($ret){
            $count=mysqli_num_rows($ret);
            if($count==0){
                echo $_POST['Cssn']."주민번호의 접종자가 없음!"."<br>";
