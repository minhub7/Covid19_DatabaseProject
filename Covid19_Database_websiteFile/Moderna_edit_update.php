...
<html lang="ko">
  ...
  <body>
    ...
    <?php
        ...
        if ( is_null( $jb_1st_vac ) ) {
          echo '<script>alert(백신 1st_DATE 오류)</script>';
        } else {
            $jb_sql = "UPDATE vaccinate SET 1st_date = '$used' WHERE 1st_Vno = '$SerialNo';";
            $ret =  mysqli_query( $jb_conn, $jb_sql );
        }

        if ( is_null( $jb_2nd_vac ) ) {
          echo '<script>alert(백신 2nd_DATE 오류)</script>';
        } else {
            $jb_sql = "UPDATE vaccinate SET 2nd_date = '$used' WHERE 2nd_Vno = '$SerialNo';";
            $ret =  mysqli_query( $jb_conn, $jb_sql );
        }
