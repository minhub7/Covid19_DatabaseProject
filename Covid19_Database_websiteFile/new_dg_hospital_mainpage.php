...
<html lang="ko">
  ...
  <?php
            ...
            if ($result['Dept'] == "government"||$result['Dept'] == "DAEGUhosp"){
            ...
            <tr>
                <td>
                    <form action="new_dg_hospital_mainpage.php" method="POST">
                        ...
                        <button class="btn" style="background-color: #029341;">대구병원</button>
                    </form>
                </td>
                <td>
...
<div class="hospital_info-box">
    <div align="center"class="title">대구병원 시민 확진/사망/격리해제 현황</div>
    <table align="center"class="hospital_info-table">
        ...
        <?php
                ...
                $jb_confirmed_sql = "SELECT count(patients.Cssn) FROM newcovid19db.confirmedcase, newcovid19db.patients WHERE patients.Cssn=confirmedcase.Tssn AND patients.Hospname='대구병원';"; 
                $jb_confirmed_result = mysqli_query( $jb_conn, $jb_confirmed_sql );

                $jb_death_sql = "SELECT count(dead_patients.Hcode) FROM newcovid19db.dead_patients,newcovid19db.patients WHERE dead_patients.Hcode=patients.Hcode AND patients.Hospname='대구병원'";         
                 $jb_death_result = mysqli_query( $jb_conn, $jb_death_sql );

                 $jb_leave_sql = "SELECT count(patients.Cssn) FROM newcovid19db.patients WHERE oDate IS NOT NULL AND patients.Hospname='대구병원'";
...
<!-- 백신 접종 테이블구현 -->
<div class="hospital_info-box">
    <div align="center"class="title">대구병원 백신 1/2차 접종 현황</div>
    <table align="center"class="hospital_info-table">
        <thead class="hospital_info-th">
                <tr>
                <td>접종 백신명</td>
    ...
    <tbody>
        <?php
                $jb_conn = mysqli_connect("localhost:3306", "root", "1234", "newcovid19db") or die("Error!!:: Covid-19 DataBase 접속 실패!!");
                $jb_pfizer1_sql = "SELECT count(vaccinate.Cssn) FROM newcovid19db.vaccinate,newcovid19db.vaccines WHERE vaccinate.1st_hosp='대구병원' AND vaccinate.1st_Vno=vaccines.serialNo AND MfgCo='pfizer'";
                $jb_pfizer1_result = mysqli_query( $jb_conn, $jb_pfizer1_sql );
                $jb_pfizer2_sql = "SELECT count(vaccinate.Cssn) FROM newcovid19db.vaccinate,newcovid19db.vaccines WHERE vaccinate.2nd_hosp='대구병원' AND vaccinate.2nd_Vno=vaccines.serialNo AND MfgCo='pfizer'";
              ...
              $jb_conn = mysqli_connect("localhost:3306", "root", "1234", "newcovid19db") or die("Error!!:: Covid-19 DataBase 접속 실패!!");
              $jb_moderna1_sql = "SELECT count(vaccinate.Cssn) FROM newcovid19db.vaccinate,newcovid19db.vaccines WHERE vaccinate.1st_hosp='대구병원' AND vaccinate.1st_Vno=vaccines.serialNo AND MfgCo='moderna'";
              $jb_moderna1_result = mysqli_query( $jb_conn, $jb_moderna1_sql );
              $jb_moderna2_sql = "SELECT count(vaccinate.Cssn) FROM newcovid19db.vaccinate,newcovid19db.vaccines WHERE vaccinate.2nd_hosp='대구병원' AND vaccinate.2nd_Vno=vaccines.serialNo AND MfgCo='moderna'";
   ...
   <table align="center"class="hospital_info-table">
        ...
        <?php
                $jb_conn = mysqli_connect("localhost:3306", "root", "1234", "newcovid19db") or die("Error!!:: Covid-19 DataBase 접속 실패!!");
                $jb_moderna_sql_all = "SELECT count(MfgCo) FROM vaccines LEFT OUTER JOIN vaccinate ON (vaccines.SerialNo = vaccinate.1st_Vno OR vaccines.SerialNo = vaccinate.2nd_Vno) WHERE Owned = '대구병원' AND MfgCo = 'moderna' AND MfgDate <= CURDATE() AND Exp >= CURDATE() AND (1st_date IS NULL AND 2nd_date IS NULL)";
                ...
                $jb_conn = mysqli_connect("localhost:3306", "root", "1234", "newcovid19db") or die("Error!!:: Covid-19 DataBase 접속 실패!!");
                $jb_pfizer_sql = "SELECT count(MfgCo) FROM vaccines LEFT OUTER JOIN vaccinate ON (vaccines.SerialNo = vaccinate.1st_Vno OR vaccines.SerialNo = vaccinate.2nd_Vno) WHERE Owned = '대구병원' AND MfgCo = 'pfizer' AND MfgDate <= CURDATE() AND Exp >= CURDATE() AND (1st_date IS NULL AND 2nd_date IS NULL)";
