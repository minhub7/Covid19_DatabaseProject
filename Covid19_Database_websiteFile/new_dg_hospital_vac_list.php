            ...
            <tr>
                ...
                <td>
                    <form action="new_dg_hospital_vac_list.php" method="POST">
                        <input type="hidden" name="uid" value="<?=$_POST['uid']?>"/>
                        <button class="btn"  style="background-color: #029341">백신 리스트</button>
                    </form>
                </td>
                <td>
   ...
   <div class="hospital_info-box">
    ...
    <table align="center"class="hospital_info-table">
        ...
        <?php
                ...
                $jb_moderna_sql_all = "SELECT * FROM vaccines LEFT OUTER JOIN vaccinate ON (vaccines.SerialNo = vaccinate.1st_Vno OR vaccines.SerialNo = vaccinate.2nd_Vno) WHERE Owned = '대구병원' AND MfgCo = 'moderna' AND MfgDate <= CURDATE() AND Exp >= CURDATE() AND (1st_date IS NULL AND 2nd_date IS NULL)";
...
<div class="hospital_info-box">
    ...
    <table align="center"class="hospital_info-table">
        ...
        <?php
            $username=$_POST['uid'];
                 $jb_conn = mysqli_connect("localhost:3306", "root", "1234", "newcovid19db") or die("Error!!:: Covid-19 DataBase 접속 실패!!");
                 $jb_pfizer_sql = "SELECT * FROM vaccines LEFT OUTER JOIN vaccinate ON (vaccines.SerialNo = vaccinate.1st_Vno OR vaccines.SerialNo = vaccinate.2nd_Vno) WHERE Owned = '대구병원' AND MfgCo = 'pfizer' AND MfgDate <= CURDATE() AND Exp >= CURDATE() AND (1st_date IS NULL AND 2nd_date IS NULL)";
