    ...
    <?php
        ...
        if ($ret[0] ==1){
            if($same_patient_cssn[0] == 1){
                ...
                echo "<form action='new_dg_patients_new.php' method='POST'>
            ...
            if($same_tester[0] != 1){
                $jb_sql = "INSERT INTO testers VALUES('$Cssn', CURDATE(), 1, '대구병원')";
                $ret =  mysqli_query( $jb_conn, $jb_sql );
            }
            $jb_sql = "INSERT INTO confirmedcase VALUES('$Cssn', CURDATE(), 'admission')";
            $ret =  mysqli_query( $jb_conn, $jb_sql );
            if($Odate==NULL){
                $jb_sql = "INSERT INTO patients VALUES('대구병원', '$Hcode', '$Cssn', '$Building', '$Room', '$Hdate',NULL,'$Tel_nok')";
            }else{
                $jb_sql = "INSERT INTO patients VALUES('대구병원', '$Hcode', '$Cssn', '$Building', '$Room', '$Hdate','$Odate','$Tel_nok')";
            }
            $ret =  mysqli_query( $jb_conn, $jb_sql );
